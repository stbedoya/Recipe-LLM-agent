...
class PhoneNumber(str):
    ...
    @classmethod
    def _validate(
        cls, phone_number: str, _: core_schema.ValidationInfo
    ) -> str:
        try:
            parsed_number = phonenumbers.parse(
                phone_number, cls.default_region_code
            )
        ...
        if cls.supported_regions and not any(
            phonenumbers.is_valid_number_for_region(
                parsed_number, region_code=region
            )
            for region in cls.supported_regions
        ):
            raise PydanticCustomError(
                "value_error", "value is not from a supported region"
            )

        return phonenumbers.format_number(
            parsed_number,
            getattr(phonenumbers.PhoneNumberFormat, cls.phone_format),
        )
...
class PhoneNumberValidator:
    ...
    def __post_init__(self) -> None:
        if (
            self.default_region
            and self.default_region not in phonenumbers.SUPPORTED_REGIONS
        ):
            raise ValueError(
                f"Invalid default region code: {self.default_region}"
            )
        ...
        if self.supported_regions:
            for supported_region in self.supported_regions:
                if supported_region not in phonenumbers.SUPPORTED_REGIONS:
                    raise ValueError(
                        f"Invalid supported region code: {supported_region}"
                    )
    ...
    def _parse(
        ...
        return phonenumbers.format_number(
            parsed_number,
            getattr(phonenumbers.PhoneNumberFormat, number_format),
        )
