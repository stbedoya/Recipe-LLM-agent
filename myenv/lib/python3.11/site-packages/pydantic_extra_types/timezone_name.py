...
def get_timezones() -> set[str]:
    ...
    if _is_available("zoneinfo") and _is_available(
        "tzdata"
    ):  # pragma: no cover
...
class TimeZoneNameSettings(type):
    def __new__(
        ...
        **kwargs: Any,
        ...
        return cast(
            "type[TimeZoneName]", super().__new__(cls, name, bases, dct)
        )
...
class TimeZoneName(str):
    ...
    def _validate(
        ...
        """
        if (
            __input_value not in cls.allowed_values
        ):  # be fast for the most common case
            if not cls.strict:
                upper_value = __input_value.strip().upper()
                if upper_value in cls.allowed_values_upper_to_correct:
                    return cls(
                        cls.allowed_values_upper_to_correct[upper_value]
                    )
