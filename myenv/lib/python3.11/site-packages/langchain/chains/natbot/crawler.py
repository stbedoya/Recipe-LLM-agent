...
class Crawler:
    ...
    **Security Note**: This is an implementation of a crawler that uses a browser via
    ...
    def click(self, id: Union[str, int]) -> None:
        # Inject javascript into the page which removes the target= attribute
        # from all links
        js = """
    ...
    def crawl(self) -> List[str]:
        ...
        def add_to_hash_tree(
            ...
            # even if the anchor is nested in another anchor, we set the "root"
            # for all descendants to be ::Self
            if node_name == tag:
                ...
                # not a descendant of an anchor, most likely it will become
                # text, an interactive element or discarded
                value = (
                    False,
                    None,
                )
        ...
        for index, node_name_index in enumerate(node_names):
            ...
            try:
                # todo replace this with proper cursoring, ignoring the fact
                # this is O(n^2) for the moment
            ...
            # inefficient to grab the same set of keys for kinds of objects,
            ...
            if node_value[index] >= 0:
                ...
                # commonly used as a separator, does not add much context -
                # lets save ourselves some token space
        ...
        # lets filter further to remove anything that does not hold any text
        # nor has click handlers + merge text from leaf#text nodes with the
        # parent
