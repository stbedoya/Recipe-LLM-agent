...
def message_chunk_to_message(chunk: BaseMessageChunk) -> BaseMessage:
    ...
    if not isinstance(chunk, BaseMessageChunk):
        return chunk
    # chunk classes always have the equivalent non-chunk class as their first
    # parent
    ignore_keys = ["type"]
...
def _create_message_from_message_type(
    ...
    **additional_kwargs: Any,
    ...
    if additional_kwargs:
        if response_metadata := additional_kwargs.pop(
        ...
        additional_kwargs.update(
            additional_kwargs.pop("additional_kwargs", {})
        )
    if id is not None:
        kwargs["id"] = id
    ...
    if message_type in ("human", "user"):
        if example := kwargs.get("additional_kwargs", {}).pop(
            "example", False
        ):
...
def merge_message_runs(
    ...
    **NOTE**: ToolMessages are not merged, as each has a distinct tool call id that
    ...
    for msg in messages:
        ...
        if not last:
            merged.append(msg)
        elif isinstance(msg, ToolMessage) or not isinstance(
            msg, last.__class__
        ):
            ...
            if text_format == "string" and all(
                ...
                for j, block in enumerate(message.content):
                    ...
                    # Anthropic and Bedrock converse format
                        ...
                        # Recurse to make sure tool message contents are OpenAI
                        # format.
