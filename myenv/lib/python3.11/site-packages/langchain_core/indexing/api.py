...
def _delete(
    ...
    if isinstance(vector_store, VectorStore):
        ...
        if (
            "num_failed" in delete_response
            and delete_response["num_failed"] > 0
        ):
...
def index(
    ...
    *,
    ...
    if (
        cleanup == "incremental" or cleanup == "scoped_full"
    ) and source_id_key is None:
    ...
    for doc_batch in _batch(batch_size, doc_iterator):
        ...
        if cleanup == "incremental":
            # Get the uids of the documents that were not returned by the
            # loader.
