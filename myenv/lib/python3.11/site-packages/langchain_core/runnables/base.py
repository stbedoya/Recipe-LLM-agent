    ...
    **RunnableParallel** invokes runnables concurrently, providing the same input
    ...
    async def abatch_as_completed(
        ...
        **kwargs: Optional[Any],
        ...
        max_concurrency = (
            configs[0].get("max_concurrency") if configs else None
        )
        semaphore = (
    ...
    def with_config(
        self,
        config: Optional[RunnableConfig] = None,
        # Sadly Unpack is not well-supported by mypy so this will have to be
        # untyped
        **kwargs: Any,
        ...
        **kwargs: Optional[Any],
        ...
        try:
            ...
            if asyncio_accepts_context():
                # type: ignore
                output: Output = await asyncio.create_task(
    ...
    def _transform_stream_with_config(
        ...
        **kwargs: Optional[Any],
        ...
        """
        # Mixin that is used by both astream log and astream events
        # implementation
        from langchain_core.tracers._streaming import _StreamingCallbackHandler

        # tee the input so we can iterate over it twice
        input_for_tracing, input_for_transform = tee(input, 2)
        # Start the input iterator to ensure the input Runnable starts before
        # this one
        ...
        try:
            ...
            iterator = context.run(
                transformer, input_for_transform, **kwargs
    ...
    **RunnableSequence** is the most important composition operator in LangChain
            ...
            if return_exceptions:
                ...
                for stepidx, step in enumerate(self.steps):
                    ...
                    inputs = step.batch(
                        ...
                        [
                            # each step a child run of the corresponding root
                            # run
                            patch_config(
                                config,
                ...
                # Reassemble the outputs, inserting Exceptions for failed
                # inputs
    ...
    def _transform(
        ...
        **kwargs: Any,
        ...
        # buffer input in memory until all available, and then start emitting
        # output
...
class RunnableParallel(RunnableSerializable[Input, dict[str, Any]]):
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        try:
            # copy to avoid issues from the caller mutating the steps during
            # invoke()
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        for output in self.stream(input, config, **kwargs):
            final = (
                output if final is None else final + output
            )  # type: ignore[operator]
        ...
        if isinstance(func, itemgetter):
            ...
            if all(
                ...
                return create_model_v2(
                    ...
                    # To create the schema, we need to provide the module
                    # where the underlying function is defined.
                    # This allows pydantic to resolve type annotations
                    # appropriately.
                    module_name=module,
                )
    ...
    def __repr__(self) -> str:
        ...
        if self._repr is None:
            if hasattr(self, "func") and isinstance(self.func, itemgetter):
                self._repr = (
                    f"RunnableLambda({str(self.func)[len('operator.') :]})"
                )
            elif hasattr(self, "func"):
                self._repr = (
                    f"RunnableLambda({get_lambda_source(self.func) or '...'})"
                )
            elif hasattr(self, "afunc"):
...
class RunnableBindingBase(RunnableSerializable[Input, Output]):
    ...
    config: RunnableConfig = Field(
        default_factory=RunnableConfig
    )  # type: ignore
