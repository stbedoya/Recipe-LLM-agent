...
class TextInputHandler:
    ...
    def fix_cursor(self) -> Tuple[Control, ...]:
        return (
            Control.move_to_column(self._cursor_offset + self.cursor_position),
        )
...
class LiveInput(ABC, TextInputHandler):
    ...
    def _refresh(self, show_result: bool = False) -> None:
        renderable = (
            self.render_result() if show_result else self.render_input()
        )
...
class Input(LiveInput):
    ...
    def render_input(self) -> Group:
        ...
        text = (
            f"[text]{text}[/]" if self.text else f"[placeholder]{default }[/]"
        )
