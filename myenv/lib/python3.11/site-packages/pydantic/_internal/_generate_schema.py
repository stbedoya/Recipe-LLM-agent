...
class GenerateSchema:
    ...
    def _ip_schema(self, tp: Any) -> CoreSchema:
        ...
        def ser_ip(
            ip: Any, info: core_schema.SerializationInfo
        ) -> str | IpType:
    ...
    def _generate_schema_from_property(
        ...
        """
        # avoid calling `__get_pydantic_core_schema__` if we've already visited
        # this object
        ...
        if (
            ...
            and (
                existing_schema := obj.__dict__.get("__pydantic_core_schema__")
            )
            is not None
            ...
            # we have no existing schema information on the property, exit
            # early so that we can go generate a schema
            return None
        ...
        if args:
            if sys.version_info >= (3, 9):
                ...
                if isinstance(obj, GenericAlias):
                    # PEP 585 generic aliases don't convert args to
                    # ForwardRefs, unlike `typing.List/Dict` etc.
                    args = (
                        (
                            _typing_extra._make_forward_ref(a)
                            if isinstance(a, str)
                            else a
                        )
                        for a in args
    ...
    def _common_field_schema(  # C901
        ...
        if not field_info.evaluated:
            ...
            if not has_instance_in_type(evaluated_type, PydanticRecursiveRef):
                ...
                # Handle any field info attributes that may have been obtained
                # from now-resolved annotations
                for k, v in new_field_info._attributes_set.items():
                    ...
                    # actually end up as attributes of the `FieldInfo`
                    # instance.
                    if (
                ...
                # Finally, ensure the field info also reflects all the
                # `_attributes_set` that are actually metadata.
                field_info.metadata = [
        ...
        with self.model_type_stack.push(
            ...
            with self._ns_resolver.push(
                ...
                dc_schema = core_schema.dataclass_schema(
                    ...
                    # pass along the frozen config setting to the pydantic-core
                    # schema
    ...
    def _unsubstituted_typevar_schema(
        ...
        try:
            ...
            # could still have a default if it's an old version of
    ...
    def _get_prepare_pydantic_annotations_for_known_type(
        ...
        # not always called from match_type, but sometimes from
        # _apply_annotations
...
def _extract_get_pydantic_json_schema(
    ...
    # handle GenericAlias' but ignore Annotated which "lies" about its origin
    # (in this case it would be `int`)
...
class _Definitions:
    ...
    def get_schema_or_ref(
        ...
        """
        ref = get_type_ref(tp)
        # return the reference if we're either (1) in a cycle or (2) it was
        # already defined
