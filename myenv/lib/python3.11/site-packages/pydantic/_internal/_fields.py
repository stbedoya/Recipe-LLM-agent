                ...
                for b in bases:
                    if hasattr(b, ann_name):
                        if not (
                            ...
                            and ann_name
                            in getattr(b, "__pydantic_fields__", {})
                        ):
                            raise NameError(
        ...
        try:
            ...
            if ann_name in annotations:
                ...
                if ann_name in parent_fields_lookup:
                    ...
                    # copy the field to make sure typevar substitutions don't
                    # cause issues with the base classes
                    field_info = copy(parent_fields_lookup[ann_name])
                else:
                    # The field was not found on any base classes; this seems to be caused by fields not getting
                    # generated thanks to models not being fully defined while initializing recursive models.
                    # Nothing stops us from just creating a new FieldInfo for
                    # this type hint, so we do this.
                    field_info = FieldInfo_.from_annotation(ann_type)
                    field_info.evaluated = evaluated
        else:
            _warn_on_nested_alias_in_annotation(ann_type, ann_name)
            if isinstance(default, FieldInfo_) and ismethoddescriptor(
                default.default
            ):
                ...
                # in the future (by simply checking for
                # `hasattr(default.default, '__get__')`).
