...
def define_expected_missing_refs(
    ...
    if not allowed_missing_refs:
        ...
        # this is a common case (will be hit for all non-generic models), so
        # it's worth optimizing for
        return None
    ...
    if expected_missing_refs:
        definitions: list[core_schema.CoreSchema] = [
            core_schema.invalid_schema(ref=ref)
...
class _WalkCoreSchema:
    ...
    def handle_definitions_schema(
        ...
        for definition in schema["definitions"]:
            if "schema_ref" in definition and "ref" in definition:
                ...
                # We want to keep such references around for implications
                # related to JSON schema, etc.:
        ...
        if not new_definitions and len(schema) == 3:
            # This means we'd be returning a "trivial" definitions schema that
            # just wrapped the inner schema
    ...
    # TODO duplicate schema types for serializers and validators, needs to be
    # deduplicated:
...
def walk_core_schema(
    ...
    """
    return f(
        schema.copy() if copy else schema,
        _dispatch if copy else _dispatch_no_copy,
    )
...
def simplify_schema_references(
    ...
    def inline_refs(
        ...
        # Assume there are no infinite loops, because we already checked for
        # that in `count_refs`
        while s["type"] == "definition-ref":
            ...
            if can_be_inlined(s, ref):
                # Inline the reference by replacing the reference with the
                # actual schema
