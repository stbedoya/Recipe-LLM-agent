...
def is_hashable(tp: Any, /) -> bool:
    ...
    return (
        tp is collections.abc.Hashable
        or get_origin(tp) is collections.abc.Hashable
    )
...
def is_callable(tp: Any, /) -> bool:
    ...
    # hence the second check:
    return (
        tp is collections.abc.Callable
        or get_origin(tp) is collections.abc.Callable
    )
...
def parent_frame_namespace(
    *, parent_depth: int = 2, force: bool = False
    ...
    # if this becomes a problem, we could implement some sort of frozen
    # mapping structure to enforce this
    if force:
