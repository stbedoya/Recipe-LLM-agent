    ...
    if type_args:
        ...
        if _typing_extra.origin_is_union(origin_type):
            ...
            resolved_type_args = tuple(
                ...
                if not (
                    _typing_extra.is_no_return(arg)
                    or _typing_extra.is_never(arg)
                )
            )
...
def has_instance_in_type(type_: Any, isinstance_target: Any) -> bool:
    ...
    if (
        ...
        # On Python < 3.10, typing_extensions implements `ParamSpec` as a
        # subclass of `list`:
...
def set_cached_generic_type(
    ...
    if len(typevar_values) == 1:
        _GENERIC_TYPES_CACHE[_early_cache_key(parent, typevar_values[0])] = (
            type_
        )
    if origin and args:
        _GENERIC_TYPES_CACHE[_late_cache_key(origin, args, typevar_values)] = (
            type_
        )
