...
class BaseSettings(BaseModel):
    ...
    def _settings_build_values(
        ...
        cli_parse_none_str = (
            cli_parse_none_str
            if not env_parse_none_str
            else env_parse_none_str
        )
        cli_hide_none_type = (
            _cli_hide_none_type
        ...
        if not any(
            [
                ...
                if isinstance(source, CliSettingsSource)
            ]
        ):
...
class CliApp:
    ...
    @staticmethod
    def _run_cli_cmd(
        model: Any, cli_cmd_method_name: str, is_required: bool
    ) -> Any:
        ...
        # If the method is asynchronous, we handle its execution based on the
        # current event loop status.
        if inspect.iscoroutinefunction(command):
            ...
            # 2. If an event loop is already running in the current thread, run
            # the coroutine in a separate thread to avoid conflicts.
            try:
            ...
            if loop and loop.is_running():
                ...
                # Running asyncio.run() here would cause conflicts, so we use a
                # separate thread.
                exception_container = []

                def run_coro() -> None:
                    try:
                        # Execute the coroutine in a new event loop in this
                        # separate thread.
