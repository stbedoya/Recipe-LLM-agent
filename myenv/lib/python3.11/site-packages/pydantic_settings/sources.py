...
if TYPE_CHECKING:
    ...
    PydanticModel = TypeVar(
        "PydanticModel", bound=PydanticDataclass | BaseModel
    )
else:
    yaml = None
    tomllib = None
...
class PydanticBaseSettingsSource(ABC):
    ...
    def _set_settings_sources_data(
    ...
    def prepare_field_value(
        ...
        if value is not None and (
            self.field_is_complex(field) or value_is_complex
        ):
...
class DefaultSettingsSource(PydanticBaseSettingsSource):
    ...
    def get_field_value(
        ...
        # Nothing to do here. Only implement the return statement to make mypy
        # happy
        return None, "", False
    ...
    def __init__(
        ...
        for field_name, field_info in settings_cls.model_fields.items():
            ...
            if init_kwarg_name:
                ...
                self.init_kwargs[preferred_alias] = init_kwargs[
                    init_kwarg_name.pop()
                ]
...
class PydanticBaseEnvSettingsSource(PydanticBaseSettingsSource):
    def __init__(
        ...
        self.env_prefix = (
            ...
            else self.config.get("env_prefix", "")
        )
    ...
    def _extract_field_info(
        ...
        if v_alias:
            if isinstance(v_alias, list):  # AliasChoices, AliasPath
                ...
                field_info.append(
                    (v_alias, self._apply_case_sensitive(v_alias), False)
                )
        ...
        if not v_alias or self.config.get("populate_by_name", False):
            if origin_is_union(
                ...
                field_info.append(
                    (
                        field_name,
                        self._apply_case_sensitive(
                            self.env_prefix + field_name
                        ),
                        True,
                    )
                )
    ...
    def _replace_field_names_case_insensitively(
        ...
        for name, value in field_values.items():
            ...
            # Item "None" of "Optional[Type[Any]]" has no attribute
            # "model_fields"
            ...
            if field_value is not None:
                if self.env_parse_none_str is not None:
                    if isinstance(field_value, dict):
                        field_value = self._replace_env_none_type_values(
                            field_value
                        )
...
class SecretsSettingsSource(PydanticBaseEnvSettingsSource):
    ...
    def __init__(
        ...
        self.secrets_dir = (
            ...
            else self.config.get("secrets_dir")
        )
        ...
        for field_key, env_name, value_is_complex in self._extract_field_info(
            ...
            for secrets_path in reversed(self.secrets_paths):
                path = self.find_case_path(
                    secrets_path, env_name, self.case_sensitive
                )
                if not path:
...
class EnvSettingsSource(PydanticBaseEnvSettingsSource):
        ...
        * Deserialize value to python object for complex field.
        ...
        if is_complex or value_is_complex:
            if isinstance(value, EnvNoneType):
                ...
                # explode_env_vars
                env_val_built = self.explode_env_vars(
                    field_name, field, self.env_vars
                )
                if env_val_built:
                    return env_val_built
            else:
                # field is complex and there's a value, decode that as JSON,
                # then add explode_env_vars
                try:
    ...
    def next_field(
        ...
        annotation = (
            field.annotation if isinstance(field, FieldInfo) else field
        )
    ...
    def explode_env_vars(
        ...
        for env_name, env_val in env_vars.items():
            ...
            *keys, last_key = env_name_without_prefix.split(
            ...
            for key in keys:
                target_field = self.next_field(
                    target_field, key, self.case_sensitive
                )
            ...
            target_field = self.next_field(
                target_field, last_key, self.case_sensitive
            )

            # check if env_val maps to a complex field and if so, parse the
            # env_val
...
class DotEnvSettingsSource(EnvSettingsSource):
    ...
    def __call__(self) -> dict[str, Any]:
        ...
        for env_name, env_value in self.env_vars.items():
            ...
            env_used = False
            for field_name, field in self.settings_cls.model_fields.items():
                for _, field_env_name, _ in self._extract_field_info(
                    field, field_name
                ):
            ...
            if not env_used:
                if is_extra_allowed and env_name.startswith(self.env_prefix):
                    # env_prefix should be respected and removed from the
                    # env_name
...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    def __init__(
        ...
        if not cli_parse_none_str:
            cli_parse_none_str = (
                "None" if self.cli_avoid_json is True else "null"
            )
    ...
    def __call__(
        ...
        *,
        ...
        if args is not None and parsed_args is not None:
            raise SettingsError(
                "`args` and `parsed_args` are mutually exclusive"
            )
        ...
        *,
        ...
        for field_name, val in parsed_args.items():
            if isinstance(val, list):
                parsed_args[field_name] = self._merge_parsed_list(
                    val, field_name
                )
            elif field_name.endswith(":subcommand") and val is not None:
                subcommand_name = field_name.split(":")[0] + val
                subcommand_dest = self._cli_subcommands[field_name][
                    subcommand_name
                ]
    ...
    def _merge_parsed_list(
        ...
        try:
                ...
                while val:
                    ...
                    if val.startswith(","):
                        ...
                        if val.startswith("{") or val.startswith("["):
                            val = self._consume_object_or_array(
                                val, merged_list
                            )
                        else:
                            try:
            ...
            raise SettingsError(
                f"Parsing error encountered for {field_name}: {e}"
            )
    ...
    def _consume_object_or_array(
        self, item: str, merged_list: list[str]
    ) -> str:
        count = 1
    ...
    def _consume_string_or_number(
        ...
        while consumed < len(item):
            if item[consumed] == '"' and (
                consumed == 0 or item[consumed - 1] != "\\"
            ):
    ...
    def _get_sub_models(
        ...
        for type_ in field_types:
            ...
            ) or is_pydantic_dataclass(_strip_annotated(type_)):
    ...
    def _sort_arg_fields(
        ...
        for field_name, field_info in _get_model_fields(model).items():
            if _CliSubCommand in field_info.metadata:
                ...
                self._verify_cli_flag_annotations(
        ...
        if positional_variadic_arg:
            if len(positional_variadic_arg) > 1:
                ...
                raise SettingsError(
                    f"{model.__name__} has multiple variadic positonal arguments: {field_names}"
                )
            elif subcommand_args:
                field_names = ", ".join(
                    [
                        name
                        for name, info in positional_variadic_arg
                        + subcommand_args
                    ]
                )
                raise SettingsError(
        ...
        if (
            ...
            def parse_args_insensitive_method(
                ...
                return parser_method(
                    root_parser, insensitive_args, namespace
                )  # type: ignore
    ...
    def _connect_root_parser(
        ...
        self._cli_subcommands: defaultdict[str, dict[str, str]] = defaultdict(
            dict
        )
    ...
    def _add_parser_args(
        ...
        # Ignore model default if the default is a model and not a subclass of
        # the current model.
        model_default = (
            None
            if (
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if _CliSubCommand in field_info.metadata:
                ...
                kwargs["dest"] = (
                    ...
                    # Related
                    # https://github.com/pydantic/pydantic-settings/pull/25
                ...
                if is_append_action:
                    ...
                    if _annotation_contains_types(
                        ...
                        self._cli_dict_args[kwargs["dest"]] = (
                            field_info.annotation
                        )
    ...
    def _convert_bool_flag(
        ...
        if kwargs["metavar"] == "bool":
            ...
            if sys.version_info >= (3, 9) or isinstance(default, bool):
                if (
                    self.cli_implicit_flags
                    or _CliImplicitFlag in field_info.metadata
    ...
    def _convert_positional_arg(
        ...
        # conjunction with model_default instead of the derived
        # kwargs['required'].
        is_required = (
            field_info.is_required() and model_default is PydanticUndefined
        )
    ...
    def _add_parser_submodels(
        ...
        model_group_kwargs["_is_cli_mutually_exclusive_group"] = any(
            issubclass(model, CliMutuallyExclusiveGroup)
            for model in sub_models
        )
        if (
            ...
            raise SettingsError(
                "cannot use union with CliMutuallyExclusiveGroup"
            )
        ...
        if not self.cli_avoid_json:
            ...
            self._add_argument(
                model_group,
                *(f"{flag_prefix}{name}" for name in arg_names),
                **kwargs,
            )
        for model in sub_models:
            self._add_parser_args(
                ...
                alias_prefixes=[
                    f"{arg_prefix}{name}." for name in alias_names[1:]
                ],
                model_default=model_default,
            )
    ...
    def _add_parser_alias_paths(
        ...
        if alias_path_args:
            ...
            arg_prefix = (
                arg_prefix[:-1] if is_nested_alias_path else arg_prefix
            )
    ...
    def _get_modified_args(self, obj: Any) -> tuple[str, ...]:
        if not self.cli_hide_none_type:
            ...
            return tuple(
                [type_ for type_ in get_args(obj) if type_ is not type(None)]
            )
    ...
    def _metavar_format_recurse(self, obj: Any) -> str:
        ...
        if _is_function(obj):
            # If function is locally defined use __name__ instead of
            # __qualname__
            return (
                obj.__name__
        ...
        if origin_is_union(get_origin(obj)):
            return self._metavar_format_choices(
                list(
                    map(
                        self._metavar_format_recurse,
                        self._get_modified_args(obj),
                    )
                )
            )
    ...
    def _help_format(
        ...
        if field_info.is_required() and model_default in (
            PydanticUndefined,
            None,
        ):
            if _CliPositionalArg not in field_info.metadata:
                ...
                _help += (
                    f" ({ifdef}required)" if _help else f"({ifdef}required)"
                )
        else:
            ...
            elif model_default not in (
                PydanticUndefined,
                None,
            ) and _is_function(model_default):
                default = (
                    f"(default factory: {self._metavar_format(model_default)})"
                )
...
def parse_env_vars(
    ...
    return {
        _get_env_var_key(k, case_sensitive): _parse_env_none_str(
            v, parse_none_str
        )
...
def _annotation_is_complex(
    annotation: type[Any] | None, metadata: list[Any]
) -> bool:
    ...
    try:
        if annotation is not None and issubclass(annotation, RootModel):
            ...
            if (
                root_annotation is not None
                and root_annotation != "RootModelRootType"
            ):
...
def _annotation_is_complex_inner(annotation: type[Any] | None) -> bool:
    ...
    return lenient_issubclass(
        annotation,
        (BaseModel, Mapping, Sequence, tuple, set, frozenset, deque),
...
def _annotation_enum_name_to_val(
    annotation: type[Any] | None, name: Any
) -> Any:
    for type_ in (annotation, get_origin(annotation), *get_args(annotation)):
        if lenient_issubclass(type_, Enum):
...
def _get_model_fields(model_cls: type[Any]) -> dict[str, FieldInfo]:
    if is_pydantic_dataclass(model_cls) and hasattr(
    ...
    if not any((field_info.alias, field_info.validation_alias)):
        ...
        for alias_path in new_alias_paths:
            ...
            alias_path_args[name] = (
                "dict" if len(alias_path.path) > 2 else "list"
            )
