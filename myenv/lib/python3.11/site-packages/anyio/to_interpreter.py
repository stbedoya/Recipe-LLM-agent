...
UNBOUND: Final = (
    2  # I have no clue how this works, but it was used in the stdlib
)
FMT_UNPICKLED: Final = 0
FMT_PICKLED: Final = 1
DEFAULT_CPU_COUNT: Final = 8  # this is just an arbitrarily selected value
# seconds a subinterpreter can be idle before becoming eligible for pruning
...
class Worker:
    ...
    def initialize(self) -> None:
        ...
        self._queue_id = queues.create(
            2, FMT_UNPICKLED, UNBOUND
    ...
    def _call(
        ...
        queues.put(
            self._queue_id, payload, FMT_PICKLED, UNBOUND
        )  # type: ignore[call-arg]

        res: Any
        is_exception: bool
        if exc_info := interpreters.exec(
            self._interpreter_id, self._run_func
        ):  # type: ignore[func-returns-value,arg-type]
