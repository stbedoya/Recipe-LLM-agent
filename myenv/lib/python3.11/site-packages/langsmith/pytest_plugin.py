...
LangSmith URL: [bright_cyan]{self.test_suite_urls[suite_name]}[/bright_cyan]"""  # noqa: E501
        ...
        for pid, status in suite_statuses.items():
            duration = status.get("end_time", now) - status.get(
                "start_time", now
            )
        ...
        passed_count = sum(
            s.get("status") == "passed" for s in suite_statuses.values()
        )
        failed_count = sum(
            s.get("status") == "failed" for s in suite_statuses.values()
        )

        # You could arrange a row to show the aggregated data—here, in the last
        # column:
        ...
        max_dynamic_col_width = (
            self.console.width - (max_status + max_duration)
        ) // 5
...
def pytest_configure(config):
    ...
    if config.getoption("--langsmith-output"):
        ...
        config.pluginmanager.register(
            LangSmithPlugin(), "langsmith_output_plugin"
        )
