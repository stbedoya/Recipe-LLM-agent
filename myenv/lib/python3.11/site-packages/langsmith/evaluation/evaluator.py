...
class DynamicRunEvaluator(RunEvaluator):
    ...
    def __init__(
        ...
        if afunc:
            afunc = _normalize_evaluator_func(
                afunc
...
def _normalize_evaluator_func(
    ...
    all_args = [
        pname for pname, p in sig.parameters.items() if p.kind != p.VAR_KEYWORD
    ]
    args_with_defaults = [
        pname
    ...
    if not all_args or (
        not all(
            pname in supported_args or pname in args_with_defaults
            for pname in all_args
        )
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def awrapper(
                ...
                arg_map = {
                    ...
                    "attachments": (
                        example.attachments or {} if example else {}
                    ),
                    "reference_outputs": (
                        example.outputs or {} if example else {}
                    ),
                }
                kwargs = {}
                args = []
                for param_name, param in sig.parameters.items():
                    # Could have params with defaults that are not in the arg
                    # map
...
def _normalize_comparison_evaluator_func(
    ...
    Callable[
        [Sequence[Run], Optional[Example]], Awaitable[_COMPARISON_OUTPUT]
    ],
]:
    supported_args = (
        "runs",
...
def _normalize_summary_evaluator(func: Callable) -> SUMMARY_EVALUATOR_T:
    ...
    # Sequence[Example] types, respectively.
        ...
        pname in supported_args for pname in all_args
    ) or all_args == [
        "runs",
        "examples",
    ]:
        ...
        def wrapper(
            ...
            arg_map = {
                ...
                "outputs": [run.outputs or {} for run in runs],
                "reference_outputs": [
                    example.outputs or {} for example in examples
                ],
            }
            kwargs = {}
            args = []
