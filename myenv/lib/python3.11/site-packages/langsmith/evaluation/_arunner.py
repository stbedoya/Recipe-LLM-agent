...
async def _aevaluate(
    target: Union[
        ATARGET_T, AsyncIterable[dict], Iterable[schemas.Run], Runnable
    ],
    /,
...
class _AsyncExperimentManager(_ExperimentManagerMixin):
    ...
    def _reset_example_attachments(
    ...
    async def astart(self) -> _AsyncExperimentManager:
        try:
            first_example = await aitertools.py_anext(
        ...
        with cf.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            async def score_all():
                async for current_results in self.aget_results():
                    # Yield the coroutine to be awaited later in
...
def _ensure_async_traceable(
    ...
    if not asyncio.iscoroutinefunction(target) and not _is_langchain_runnable(
        target
    ):
        if callable(target):
            raise ValueError(
    ...
    if rh.is_traceable_function(target):
        ...
        return rh.traceable(name="AsyncTarget")(
            target
...
def _aresolve_data(
    ...
    *,
    ...
    return aitertools.ensure_async_iterator(
        _resolve_data(
            data, client=client, include_attachments=include_attachments
        )
    )
