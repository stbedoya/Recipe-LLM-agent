...
def evaluate(
    ...
    **kwargs: Any,
    ...
    if isinstance(target, (str, uuid.UUID, schemas.TracerSession)):
        ...
        target_id = (
            target if isinstance(target, (str, uuid.UUID)) else target.id
        )
        logger.debug(
            f"Running evaluation over existing experiment {target_id}..."
        )
        ...
        if len(target) != 2 or not all(
            isinstance(t, (str, uuid.UUID, schemas.TracerSession))
            for t in target
        ):
            msg = (
        ...
        target_ids = [
            t if isinstance(t, (str, uuid.UUID)) else t.id for t in target
        ]
        logger.debug(
        ...
        return evaluate_comparative(
            target,
            evaluators=cast(
                Sequence[COMPARATIVE_EVALUATOR_T], evaluators or ()
            ),
...
def _load_traces(
    ...
    treemap: DefaultDict[uuid.UUID, List[schemas.Run]] = (
...
class _ExperimentManagerMixin:
    ...
    def _print_experiment_start(
        self,
        project: Optional[schemas.TracerSession],
...
class _ExperimentManager(_ExperimentManagerMixin):
    ...
    def examples(self) -> Iterable[schemas.Example]:
        if self._examples is None:
            ...
            if (
                self._reuse_attachments
                and self._attachment_raw_data_dict is None
            ):
    ...
    def start(self) -> _ExperimentManager:
        ...
        project = (
            self._get_project(first_example) if self._upload_results else None
        )
    ...
    def _apply_summary_evaluators(
        ...
        with ls_utils.ContextThreadPoolExecutor() as executor:
            project_id = (
                self._get_experiment().id if self._upload_results else None
            )
            current_context = rh.get_tracing_context()
            metadata = {
                **(current_context["metadata"] or {}),
...
def _forward(
    ...
    with rh.tracing_context(enabled="local" if not upload_results else True):
        ...
        try:
            ...
            logger.error(
                f"Error running target function: {e}",
                exc_info=True,
                stacklevel=1,
            )
...
def _include_attachments(
    ...
    positional_no_default = [
        p for p in positional_params if p.default is p.empty
    ]
    ...
    if len(positional_params) == 0:
        ...
        if [p.name for p in positional_no_default] != [
            "inputs",
            "attachments",
        ]:
            raise ValueError(
        ...
        return [p.name for p in positional_params[:2]] == [
            "inputs",
            "attachments",
        ]
...
def _extract_code_evaluator_feedback_keys(func: Callable) -> list[str]:
    ...
    def extract_evaluation_results_keys(node, variables):
        if (
            ...
            for key, value in zip(node.keys, node.values):
                if (
                    isinstance(key, (ast.Str, ast.Constant))
                    and key.s == "results"
                ):
                    if isinstance(value, ast.List):
                        ...
                        for elt in value.elts:
                            if isinstance(elt, ast.Dict):
                                for elt_key, elt_value in zip(
                                    elt.keys, elt.values
                                ):
                                    if (
                                        isinstance(
                                            elt_key, (ast.Str, ast.Constant)
                                        )
                                        and elt_key.s == "key"
                                    ):
                                        if isinstance(
...
def _to_pandas(
    ...
    return pd.DataFrame(
        _flatten_experiment_results(results, start=start, end=end)
    )
    ...
    return [
        {
            **{
                f"inputs.{k}": v
                for k, v in (x["example"].inputs or {}).items()
            },
            ...
            **{
                f"feedback.{r.key}": (
                    r.score if r.score is not None else r.value
                )
...
def _is_langchain_runnable(o: Any) -> bool:
    return bool(
        (Runnable := _import_langchain_runnable()) and isinstance(o, Runnable)
    )
