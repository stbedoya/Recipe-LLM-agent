...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        self._write_api_urls: Mapping[str, Optional[str]] = (
        ...
        # To trigger this code, set the `LANGSMITH_USE_PYO3_CLIENT` env var to
        # any value.
    ...
    def _batch_ingest_run_ops(
        ...
        for op in ops:
            if isinstance(op, SerializedRunOperation):
                ...
                logger.error(
                    "Unknown item type in tracing queue: %s", type(op)
                )
    ...
    def _multipart_ingest_ops(
        ...
        for op in ops:
            if isinstance(op, SerializedRunOperation):
                ...
                logger.error(
                    "Unknown operation type in tracing queue: %s", type(op)
                )
    ...
    def _send_multipart_req(
        self, acc: MultipartPartsAndContext, *, attempts: int = 3
    ):
        parts = acc.parts
        ...
        for api_url, api_key in self._write_api_urls.items():
            for idx in range(1, attempts + 1):
                try:
                    encoder = rqtb_multipart.MultipartEncoder(
                    ...
                    if idx == attempts:
                        logger.warning(
                            f"Failed to multipart ingest runs: {exc}"
                        )
                    else:
                        continue
                except Exception as e:
        ...
        **kwargs: Any,
        ...
        use_multipart = (
            (
                ...
                or self.compressed_traces is not None
            )
            # batch ingest requires trace_id and dotted_order to be set
    ...
    def _load_child_runs(self, run: ls_schemas.Run) -> ls_schemas.Run:
        ...
        treemap: DefaultDict[uuid.UUID, List[ls_schemas.Run]] = (
    ...
    def create_dataset(
        ...
        *,
        ...
        transformations: Optional[
            List[ls_schemas.DatasetTransformation]
        ] = None,
        metadata: Optional[dict] = None,
    ) -> ls_schemas.Dataset:
    ...
    def _prepare_multipart_data(
        ...
        if include_dataset_id:
            if not isinstance(
                examples[0], ls_schemas.ExampleUpsertWithAttachments
            ):
                raise ValueError(
        ...
        for example in examples:
            ...
            if (
                ...
                attachments_operationsb = _dumps_json(
    ...
    def create_examples(
        ...
        **kwargs: Any,
        ...
        if kwargs and (
            unsupported := set(kwargs).difference(supported_kwargs)
        ):
            raise ValueError(
    ...
    def update_example(
        ...
        *,
        ...
        attachments_operations: Optional[
            ls_schemas.AttachmentsOperations
        ] = None,
        attachments: Optional[ls_schemas.Attachments] = None,
    ) -> Dict[str, Any]:
    ...
    def update_examples(
        ...
        **kwargs: Any,
        ...
        if not (kwargs or updates):
            raise ValueError(
                "Please pass in a non-empty sequence for arg 'updates'."
            )
        ...
        # For backwards compatibility
            ...
            if not dataset_id:
                if "dataset_ids" not in kwargs:
                    ...
                    raise ValueError(
                        "Dataset IDs must be the same for all examples"
                    )
                elif not kwargs["dataset_ids"][0]:
                    raise ValueError(
                        "If specified, dataset_ids must be non-null."
                    )
                else:
            ...
            if (
                ...
                and (
                    kwargs.get("attachments_operations")
            ...
            updates_obj = [
                ...
                for id_, in_, out_, metadata_, split_, dataset_id_, attachments_, attachments_operations_ in zip(
                    ...
                    kwargs.get(
                        "metadata", (None for _ in range(examples_len))
                    ),
                    kwargs.get("splits", (None for _ in range(examples_len))),
                    kwargs.get(
                        "dataset_ids", (None for _ in range(examples_len))
                    ),
                    kwargs.get(
                        "attachments", (None for _ in range(examples_len))
                    ),
                    kwargs.get(
                        "attachments_operations",
    ...
    def create_feedback(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if (
                feedback_source.metadata
                and "__run" in feedback_source.metadata
            ):
        ...
        *,
        ...
        params = {
            ...
            "is_public": (
                "true"
                if is_public
                else "false" if is_public is not None else None
            ),
    ...
    def pull_prompt(
        ...
        if (
            ...
            and (
                len(prompt.steps) == 2
                and not isinstance(prompt.last, BaseOutputParser)
            )
        ):
    ...
    def push_prompt(
        ...
        *,
        ...
        if self._prompt_exists(prompt_identifier):
            if any(
                param is not None
                for param in [is_public, description, readme, tags]
            ):
                self.update_prompt(
                    prompt_identifier,
                    description=description,
    ...
    async def aevaluate(
        ...
        evaluators: Optional[
            Sequence[Union[EVALUATOR_T, AEVALUATOR_T]]
        ] = None,
        summary_evaluators: Optional[Sequence[SUMMARY_EVALUATOR_T]] = None,
        metadata: Optional[dict] = None,
        experiment_prefix: Optional[str] = None,
        ...
        blocking: bool = True,
        experiment: Optional[
            Union[schemas.TracerSession, str, uuid.UUID]
        ] = None,
        upload_results: bool = True,
        **kwargs: Any,
    ) -> AsyncExperimentResults:
