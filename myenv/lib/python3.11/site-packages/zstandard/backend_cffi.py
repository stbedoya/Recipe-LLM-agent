...
class ZstdCompressionParameters(object):
    ...
    def __init__(
        ...
        _set_compression_parameter(
            params, lib.ZSTD_c_dictIDFlag, write_dict_id
        )
    ...
    def target_length(self):
        return _get_compression_parameter(
            self._params, lib.ZSTD_c_targetLength
        )
    ...
    def write_checksum(self):
        return _get_compression_parameter(
            self._params, lib.ZSTD_c_checksumFlag
        )
...
class ZstdCompressionWriter(object):
    ...
    def __init__(
        ...
        zresult = lib.ZSTD_CCtx_setPledgedSrcSize(
            compressor._cctx, source_size
        )
        if lib.ZSTD_isError(zresult):
            raise ZstdError(
    ...
    def read_to_iter(
        ...
        while True:
            ...
            if have_read:
                ...
                read_result = reader[
                    buffer_offset: buffer_offset + slice_size
                ]
    ...
    def _ensure_dctx(self, load_dict=True):
        ...
        if self._dict_data and load_dict:
            zresult = lib.ZSTD_DCtx_refDDict(
                self._dctx, self._dict_data._ddict
            )
            if lib.ZSTD_isError(zresult):
                raise ZstdError(
